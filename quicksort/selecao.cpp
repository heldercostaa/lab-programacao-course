...
int* Selecao::selecionar(int *i, int *f, int k) {
	switch (selecao) {
		case FIXA:
			...
			return selecao_fixa(i, f);
			break;
		case ALEATORIA:
			if(debug) { std::cout << "Seleção: Aleatória\n"; }
			return selecao_aleatoria(i, f);
			break;
		case HOARE:
...
int* Selecao::selecao_fixa(int* i, int* f) {
	
	return i;
}

int* Selecao::selecao_aleatoria(int* i, int* f) {
	
	int fim = (int) (f-i);
	int indice_aleatorio = rand()%(fim+1);
	
	return i+indice_aleatorio;
}

int* Selecao::selecao_hoare(int* i, int* f, int k) {
	
	int* pivo = selecao_aleatoria(i, f);
